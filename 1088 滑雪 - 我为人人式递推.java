...
public class Main {
	public static void main(String[] args) {
		...
		for (Node node : nodes) {
			...
			int nodeY = node.y;
			int height = node.h;
			List<Node> neighbors = getHigherNeighbors(map, nodes, nodeX, nodeY, height, row, col);
			if (neighbors.size() != 0) {
				for (Node n : neighbors) {
					n.length = Math.max(n.length, node.length + 1);
	...
	private static List<Node> getHigherNeighbors(Node[][] map, Node[] nodes, int nodeX, int nodeY, int height, int row, int col) {
